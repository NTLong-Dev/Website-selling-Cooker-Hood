...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
                ...
                <td>
                    <form action="{{ route('cart.update', $id) }}" method="POST" style="display: inline;">
                        ...
                        <input type="number" name="quantity" value="{{ $details['quantity'] }}" min="1"
                               class="form-control" style="width: 69px; display: inline;">
                        <button type="submit" class="btn btn-sm btn-secondary">Cập nhật</button>
                    </form>
                </td>
                <td>{{ $details['quantity'] }}</td>
                <td>{{ number_format($details['price'], 2) }}</td>
                <td>{{ number_format($details['price'] * $details['quantity'], 2) }}</td> <!-- Tính thành tiền -->
    ...
    <h3>Tổng tiền: {{ number_format($total, 2) }} VND</h3> <!-- Hiển thị tổng tiền -->
</div>
        </table>
        <!-- Form đặt hàng -->
